...
def define_G(
    ...
    if netG == "resnet_9blocks":
        ...
        net = UnetGenerator(
            input_nc, output_nc, 6, ngf, norm_layer=norm_layer, use_dropout=use_dropout, gpu_ids=gpu_ids
        )
    elif netG == "unet_128":
        net = UnetGenerator(
            input_nc, output_nc, 7, ngf, norm_layer=norm_layer, use_dropout=use_dropout, gpu_ids=gpu_ids
        )
    elif netG == "unet_256":
        net = UnetGenerator(
            input_nc, output_nc, 8, ngf, norm_layer=norm_layer, use_dropout=use_dropout, gpu_ids=gpu_ids
        )
    else:
...
def define_D(
    input_nc, ndf, netD, n_layers_D=3, norm="batch", use_sigmoid=False, init_type="normal", init_gain=0.02, gpu_ids=[]
):
    net = None
    norm_layer = get_norm_layer(norm_type=norm)
...
class UnetGenerator(nn.Module):
    def __init__(
        self, input_nc, output_nc, num_downs, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, gpu_ids=[]
    ):
        super(UnetGenerator, self).__init__()

        # construct unet structure
        unet_block = UnetSkipConnectionBlock(
            ngf * 8, ngf * 8, input_nc=None, submodule=None, norm_layer=norm_layer, innermost=True, gpu_ids=gpu_ids
        )
        ...
        unet_block = UnetSkipConnectionBlock(
            ngf * 4, ngf * 8, input_nc=None, submodule=unet_block, norm_layer=norm_layer, gpu_ids=gpu_ids
        )
        unet_block = UnetSkipConnectionBlock(
            ngf * 2, ngf * 4, input_nc=None, submodule=unet_block, norm_layer=norm_layer, gpu_ids=gpu_ids
        )
        unet_block = UnetSkipConnectionBlock(
            ngf, ngf * 2, input_nc=None, submodule=unet_block, norm_layer=norm_layer, gpu_ids=gpu_ids
        )
        unet_block = UnetSkipConnectionBlock(
            output_nc,
            ngf,
...
class UnetSkipConnectionBlock(nn.Module):
    def __init__(
        ...
        if input_nc is None:
            input_nc = outer_nc
        downconv_base = nn.Conv2d(input_nc, inner_nc, kernel_size=4, stride=2, padding=1, bias=use_bias)
        downrelu_base = nn.LeakyReLU(0.2, True)
        downnorm_base = norm_layer(inner_nc)
        downconv_res = nn.Conv2d(input_nc, inner_nc, kernel_size=4, stride=2, padding=1, bias=use_bias)
