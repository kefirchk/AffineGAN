...
def get_ssim_psnr(ref_dir, pred_dir, pattern, start, end, mode="L"):
    ssim_conv = 0.0
    psnr_conv = 0.0
    ...
    for i in range(start, end):
        ...
        b = np.array(b)
        ssim_conv += structural_similarity(a, b, channel_axis=-1 if mode == "RGB" else None)
        psnr_conv += psnr(b, a)
...
def get_ACD(ref_dir, pred_dir, pattern, start, end):
    ...
    return (
        ACD_m / (end - start),
        ACD_i / (end - start),
        ACD_c / (len(pred_features) * (len(pred_features) - 1)) * 2,
        ACD_ref_i / (end - start),
        ACD_ref_c / (len(pred_features) * (len(pred_features) - 1)) * 2,
    )
